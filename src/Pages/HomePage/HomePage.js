...
const HomePage = () => {
  ...
  const groupFoldersByMonth = (folders) => {
    const grouped = folders.reduce((acc, folder) => {
      const monthYear = new Date(folder.date).toLocaleString("default", {
        month: "long",
        year: "numeric",
      });
      if (!acc[monthYear]) {
    ...
    // Sort months with December 2024 first, then future months
    return Object.keys(grouped)
      .sort((a, b) => {
        const dateA = new Date(grouped[a][0].date);
        const dateB = new Date(grouped[b][0].date);

        // Special handling for December 2024
        if (a.includes("December 2024")) return -1;
        if (b.includes("December 2024")) return 1;

        // For all other months, sort chronologically
        return dateA - dateB;
      })
      .reduce((acc, month) => {
        acc[month] = grouped[month].sort(
  ...
  const handleFolderClick = async (pathIncludingSlash) => {
    ...
    try {
      ...
      setFolderObjects(
        objectsWithMetadata.filter((obj) => !obj.Key.endsWith("/"))
      );
    } catch (error) {
          ...
          async (prefix) => {
            ...
            try {
              ...
              if (
                ...
                folderResponse.Contents.length > 0
              ) {
                // Try to find a date in any of the file names
                let folderDate = null;
                for (const file of folderResponse.Contents) {
                  const fileName = file.Key.split("/").pop();
                  folderDate = extractDateFromFileName(fileName);
                  if (folderDate) break;
                }

                // If no date found in filenames, use the LastModified date
                if (!folderDate) {
                  folderDate = new Date(
                    folderResponse.Contents[0].LastModified
                  );
                }

                return {
                  folderName,
                  files: folderResponse.Contents.map((file) => ({
                    key: file.Key,
                    lastModified: file.LastModified,
                    size: file.Size,
                  })),
                  hasUpgrades: folderResponse.Contents.some((file) =>
                    file.Key.includes("Upgrades")
                  ),
                  date: folderDate,
                };
                ...
                <div className="space-y-4">
                  {groupedFolders[monthYear].map((folder) => (
                    ...
                    >
                      ...
                      {selectedFolder === folder.folderName && (
                        <div className="mt-4">
                          {folderObjects.map((file) => {
                            const fileExtension = file.Key.split(".")
                              .pop()
                              .toLowerCase();
                            const iconSrc =
                              fileExtension === "pdf"
                            ...
                            return (
                              ...
                              >
                                ...
                                {iconSrc && (
                                  <img
                                    src={iconSrc}
                                    alt={`${fileExtension.toUpperCase()} icon`}
                                    className="w-6 h-6 mr-2"
                                  />
                                )}
                                <span>{file.Key.split("/").pop()}</span>
